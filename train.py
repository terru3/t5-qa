...
def prep_train():
   ...
   scheduler = get_scheduler(
       "linear",
       optimizer=optimizer,
       num_warmup_steps=len(train_dataloader)*NUM_EPOCH_WARMUP, # half an epoch for warmup
...
def train(model, train_dataloader, val_dataloader, 
    ...
    COMPUTE_EVERY = round(int(len(train_dataloader) / COMPUTE_PER_EPOCH),
                    -2) # nearest hundred
    print(f"There are {len(train_dataloader)} batches in an epoch; train and val losses are computed every",
          f"{COMPUTE_EVERY} steps.")

    for epoch in range(EPOCHS):
        print(f"Epoch {epoch}, Learning rate: {scheduler.optimizer.param_groups[0]['lr']:.5f}")
        epoch_loss = []
        total_loss = 0
